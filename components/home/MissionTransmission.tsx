        ...
        <div className="flex flex-col items-center">
          ...
          <motion.div 
            ...
            className={cn(
              ...
              isPlaying ? "shadow-[0_0_100px_rgba(0,0,0,1)] scale-105 z-50" : "hover:border-white/20"
            )}
          >
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-[#080808]">
              ...
              <motion.div 
                ...
                className="relative w-96 h-96 border border-cyan-500/10 rounded-full flex items-center justify-center"
              >
                <div className="absolute inset-0 border border-dashed border-cyan-500/5 rounded-full animate-[spin_60s_linear_infinite]" />
                ...
                {!isPlaying && (
                  ...
                  >
                    <div className="mb-8 flex flex-col items-center">
                      <span className="text-[10px] text-cyan-500/40 font-mono tracking-[0.5em] mb-2 uppercase">DATA_STREAM: PENDING</span>
                      <div className="h-[1px] w-32 bg-gradient-to-r from-transparent via-cyan-500/30 to-transparent" />
                    </div>
                    
                    <button 
                      onClick={() => setIsPlaying(true)}
                      className="w-24 h-24 rounded-full border border-white/10 flex items-center justify-center bg-black/40 backdrop-blur-md hover:border-cyan-500/50 hover:scale-110 transition-all duration-700 group/play"
                    >
                      <Play className="text-white/60 ml-1 fill-white/20 group-hover/play:text-cyan-500 group-hover/play:fill-cyan-500 transition-colors" size={32} />
                    </button>
                    ...
                    <motion.div 
                      ...
                      animate={{ opacity: 1 }}
                      transition={{ delay: 1 }}
                      className="mt-12 text-center max-w-lg px-6"
                    >
                      <div className="flex items-center justify-center space-x-2 mb-4">
                        <div className="w-1 h-1 bg-cyan-500 rounded-full animate-pulse" />
                        <span className="text-[10px] uppercase tracking-[0.4em] text-white/50 font-mono">MISSION_MANIFESTO_v1.0</span>
                      </div>
                      <h3 className="text-xl md:text-2xl font-light text-white/80 leading-relaxed tracking-wide text-balance">
                        "Engineering the modular infrastructure required to execute at the speed of the future."
                      </h3>
                      <div className="mt-6 flex flex-col items-center">
                        <div className="text-[8px] text-white/20 font-mono uppercase tracking-[0.3em]">INITIATE AUTHENTICATED ACCESS</div>
            ...
            <AnimatePresence>
              {(isHovered || isPlaying) && (
                <motion.div 
                  ...
                  exit={{ opacity: 0, y: 10 }}
                  className="absolute bottom-0 left-0 right-0 p-8 flex items-center justify-between z-40 bg-gradient-to-t from-black to-transparent"
                >
                  <div className="flex items-center space-x-6">
                    <button onClick={() => setIsPlaying(!isPlaying)} className="text-white/60 hover:text-white transition-colors">
                    ...
                    <span className="text-[10px] font-mono text-white/40">01:42 / 03:00</span>
                  </div>
          ...
          <AnimatePresence>
            {isPlaying && (
              <motion.div 
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="fixed inset-0 bg-black/80 z-[45] backdrop-blur-md pointer-events-none" 
              />
            )}
          </AnimatePresence>
          ...
          >
            <p className="text-white/40 text-sm md:text-base font-light leading-relaxed italic">
